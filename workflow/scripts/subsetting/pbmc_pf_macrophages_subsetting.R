...
seuratObject <- seuratObject[,seuratObject@meta.data$manual_l2 == "Macrophages" & seuratObject@meta.data$Tissue %in% c("PF", "PBMC")]
...
pf_atlas_seuratObject <- pf_atlas_seuratObject[,pf_atlas_seuratObject@meta.data$manual_l2 == "Macrophages"]
pf_atlas_seuratObject <- DietSeurat(pf_atlas_seuratObject, counts = T, data = T, scale.data = F)
...
seuratObject <- RunPCA(object = seuratObject, npcs = 100, seed.use = 697856)
...
seuratObject <- RunTSNE(seuratObject, dims = 1:48, seed.use = 56475643)
