#!/usr/bin/env Rscript
# The goal of this script is to subset the galbladder-derived macrophages from adults only.
...
cells_selected <- seuratObject@meta.data %>%
  dplyr::filter(tissue_original %in% c("AdultGallbladder"),
...
seuratObject <- RunPCA(object = seuratObject, npcs = 100, seed.use = 5254)
seuratObject <- FindNeighbors(seuratObject, reduction = "pca", dims = 1:22)
seuratObject <- FindClusters(seuratObject, resolution = 0.5, verbose = FALSE)
seuratObject <- RunUMAP(seuratObject, dims = 1:22, seed.use = 37835)
