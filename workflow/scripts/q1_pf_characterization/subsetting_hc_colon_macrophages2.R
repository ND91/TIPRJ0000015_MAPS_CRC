...
cells_selected <- seuratObject@meta.data %>%
  dplyr::filter(tissue_original %in% c("AdultStomach"),
...
seuratObject <- RunPCA(object = seuratObject, npcs = 100, seed.use = 64233)
seuratObject <- FindNeighbors(seuratObject, reduction = "pca", dims = 1:19)
seuratObject <- FindClusters(seuratObject, resolution = 0.5, verbose = FALSE)
seuratObject <- RunUMAP(seuratObject, dims = 1:19, seed.use = 543534)
