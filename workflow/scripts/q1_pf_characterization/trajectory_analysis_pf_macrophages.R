...
aggregated_centroids <- reducedDim(aggregated_clusters, "GEX.PCA")
aggregated_mst <- TSCAN::createClusterMST(aggregated_centroids, clusters = NULL)
aggregated_lines <- TSCAN::reportEdges(aggregated_clusters, mst=aggregated_mst, use.dimred="GEX.UMAP")
cellmapped_aggregatedmst <- mapCellsToEdges(sce, mst=aggregated_mst, use.dimred="GEX.PCA")
...
#colData(sce)$Pseudotime <- averagePseudotime(tscan_rootcl1) 
