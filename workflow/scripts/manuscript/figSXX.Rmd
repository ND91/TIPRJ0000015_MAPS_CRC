...
```{r clean subsets}
seurat_gc_pf <- seurat_gc_pf[,-which(seurat_gc_pf@meta.data$manual_l3 %in% c("T proliferating", "Unknown", "NK CD16- proliferating", "NK CD16+ proliferating", "CDCs proliferating", "Macrophages C1Q+ proliferating"))]
seurat_gc_pf@meta.data$manual_l2l3 <- seurat_gc_pf@meta.data %>%
  dplyr::mutate()
```
...
celltype_order_l3_tx_monocytes_macrophages <- data.frame(celltype = c("Classical monocytes", "Non-classical monocytes", "Macrophages VCAN+", "Macrophages C1Q+", "Macrophages VCAN+C1Q+", "Macrophages SPP1+"),
...
marker_gex <- GetAssayData(seurat_gc_pf_l2l3, assay = "RNA")[which(rownames(GetAssayData(seurat_gc_pf_l2l3, assay = "RNA")) %in% unique(marker_genes$FeatureID)), ]

dotplot_gc_pf_l2l3_cellmarkers_gex_ggplotobj <- data.frame(FeatureID = rownames(marker_gex), marker_gex) %>%
...
print(dotplot_gc_pf_l2l3_cellmarkers_gex_ggplotobj)

pdf("dotplot_gc_pf_l2l3_cellmarkers_gex_ggplotobj.pdf", width = 7.5, height=15)
print(dotplot_gc_pf_l2l3_cellmarkers_gex_ggplotobj)
dev.off()
```
