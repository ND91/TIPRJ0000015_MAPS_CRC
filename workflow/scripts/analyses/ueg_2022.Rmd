...
mnp_markers <- c("CD14", "FCGR3A", "CD163", "MARCO", "CD68", "CD1C")
...
mnp_seuratObject_umap_df <- data.frame(Embeddings(mnp_seuratObject[["umap"]]),
                                       ...
                                       t(GetAssayData(mnp_seuratObject)[mnp_markers,])) %>%
...
```{r mnp UMAP col_manual_l2 split_tissue}
pdf(file = "pbmc_pf_pmn_mnp_umap.pdf", width = 10, height = 6)
mnp_seuratObject_umap_df %>%
  dplyr::filter(Tissue != "TX" & Metastasis == "PM-") %>%
  ...
  labs(subtitle = "Mononuclear phagocytes from CRC patients") +
  facet_grid(.~Tissue) +
  theme_minimal() +
...
```{r mnp UMAP col_marker}
mnp_marker_list <- lapply(mnp_markers, function(marker_gene){
  mnp_seuratObject_umap_df %>%
    dplyr::filter(Tissue != "TX" & Metastasis == "PM-") %>%
    ggplot(aes(x = UMAP_1, y = UMAP_2)) +
...
pdf(file = "pbmc_pf_pmn_mnp_marker_umap.pdf", width = 15, height = 10)
ggarrange(plotlist = mnp_marker_list, nrow = 2, ncol = 3)
dev.off()
```


```{r myeloid UMAP col_manual_l2 split_tissue}
pdf(file = "pbmc_mnp_umap2.pdf", width = 15, height = 5)
mnp_seuratObject_umap_df %>%
...
### M2 macrophages: PF: PM+vPM-

```{r m2 macrophages pf pmpvpmn degs}
pf_macrophagem2_uegpmp_samples_seuratObject <- pbmc_pf_tx_seuratObject[,pbmc_pf_tx_seuratObject$manual_l3 == "M2-like" &
                                                                             pbmc_pf_tx_seuratObject@meta.data$SampleID %in% ueg_samples$SampleID &
                                                                             pbmc_pf_tx_seuratObject@meta.data$Tissue %in% c("PF")]

pf_macrophagem2_uegpmp_samples <- unique(pf_macrophagem2_uegpmp_samples_seuratObject@meta.data[,c("SampleID", "Donor", "Metastasis")])
rownames(pf_macrophagem2_uegpmp_samples) <- pf_macrophagem2_uegpmp_samples$SampleID
pf_macrophagem2_uegpmp_samples$Metastasis_rc <- ifelse(pf_macrophagem2_uegpmp_samples$Metastasis == "PM+", "PMp", "PMn")

pf_macrophagem2_uegpmp_counts <- GetAssayData(pf_macrophagem2_uegpmp_samples_seuratObject, slot = "counts")
pf_macrophagem2_uegpmp_counts_sample <- pf_macrophagem2_uegpmp_counts %*% model.matrix(~0+SampleID, data = pf_macrophagem2_uegpmp_samples_seuratObject@meta.data)
colnames(pf_macrophagem2_uegpmp_counts_sample) <- gsub("SampleID", "", colnames(pf_macrophagem2_uegpmp_counts_sample))

pf_macrophagem2_uegpmp_counts_sample <- pf_macrophagem2_uegpmp_counts_sample[which(Matrix::rowSums(pf_macrophagem2_uegpmp_counts_sample) != 0),]

pf_macrophagem2_uegpmp_dds <- DESeqDataSetFromMatrix(countData = data.frame(pf_macrophagem2_uegpmp_counts_sample),
                                                     colData = pf_macrophagem2_uegpmp_samples[colnames(pf_macrophagem2_uegpmp_counts_sample),],
                                                     design = ~Metastasis_rc)
pf_macrophagem2_uegpmp_dds <- DESeq(pf_macrophagem2_uegpmp_dds)

degs_pf_macrophagem2_uegpmp <- results(pf_macrophagem2_uegpmp_dds, name = "Metastasis_rc_PMp_vs_PMn")
degs_pf_macrophagem2_uegpmp <- degs_pf_macrophagem2_uegpmp[!is.na(degs_pf_macrophagem2_uegpmp$padj),]
degs_pf_macrophagem2_uegpmp <- degs_pf_macrophagem2_uegpmp[order(degs_pf_macrophagem2_uegpmp$pvalue),]
degs_pf_macrophagem2_uegpmp$gene <- rownames(degs_pf_macrophagem2_uegpmp)

pf_macrophagem2_uegpmp_rld <- rlog(pf_macrophagem2_uegpmp_dds)

pdf(width = 12.5, height = 10, file = file.path("m2_macrophages_pf_goi_pmpvpmn.pdf"))
assay(pf_macrophagem2_uegpmp_rld)[c1q_macrophages_goi,] %>%
  data.frame(., GeneID = rownames(.)) %>%
  tidyr::pivot_longer(-GeneID, names_to = "SampleID", values_to = "Expr") %>%
  dplyr::left_join(data.frame(colData(pf_macrophagem2_uegpmp_rld)), by = "SampleID") %>%
  dplyr::left_join(data.frame(degs_pf_macrophagem2_uegpmp), by = c("GeneID" = "gene")) %>%
  dplyr::mutate(label = paste0(GeneID, "\np-value = ", formatC(pvalue, digits = 2, format = "e"))) %>%
  ggplot(aes(x = Metastasis, y = Expr, col = Metastasis)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter() +
  labs(y = bquote(log[2]~"(expression)")) +
  facet_wrap(~label, scales = "free_y") +
