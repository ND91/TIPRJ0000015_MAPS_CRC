...
#     return_props: Boolean whether to return the transformed proportions. Defaults to FALSE.
...
seuratDA <- function(seuratobj, sampleinfo, cellsampleID, cellclusterID, clusterparentID = NULL, ncell_threshold = 20, method = c("propeller", "DESeq2", "edgeR"), design, contrast = NULL, name = NULL, return_props = F){
      ...
      tryCatch(expr = {
        if(method == "propeller"){
          ...
          if(return_props == TRUE){
            props <- speckle::getTransformedProps(clusters = seuratentry@meta.data[,cellclusterID],
                                         sample = seuratentry@meta.data[,cellsampleID], 
                                         transform = "logit")
            
            return_obj <- list(da = da, props = props)
          } else{
            return_obj <- da
          }
