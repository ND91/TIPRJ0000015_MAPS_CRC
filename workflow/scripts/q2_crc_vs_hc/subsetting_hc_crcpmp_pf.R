...
# The goal of this script is to select the PF from HC and CRC PM+ patients.
...
seurat_rds <- args[1]
hc_crcpmp_pf_seurat_rds <- args[2]
...
seuratObject <- RunPCA(object = seuratObject, npcs = 100, seed.use = 4312796)
...
seuratObject <- RunUMAP(seuratObject, dims = 1:48, seed.use = 152315)

# Save data
saveRDS(seuratObject, hc_crcpmp_pf_seurat_rds, compress = "gzip")
