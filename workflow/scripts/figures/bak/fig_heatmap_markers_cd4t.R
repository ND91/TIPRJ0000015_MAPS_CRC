...
cd4t_marker_expr <- GetAssayData(seuratObject, assay = "RNA")[which(rownames(seuratObject) %in% cd4t_marker_genes),]
...
cd4t_genecelltype_median <- data.frame(GeneID = rownames(cd4t_marker_expr), cd4t_marker_expr) %>%
  ...
  dplyr::inner_join(data.frame(CB = rownames(seuratObject@meta.data), 
                               Donor = seuratObject@meta.data$Donor,
...
svglite::svglite(width = 15, height = 15, file = "cd4t_ccr4_ccr5_ccr6_ccr8_split.svg", bg = "white")
FeaturePlot(seuratObject, c("CCR4", "CCR5", "CCR6", "CCR8"), label = T, raster = T, split.by = "Tissue")
dev.off()
